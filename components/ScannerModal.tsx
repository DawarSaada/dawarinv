      ...
      try {
        ...
        if (cameras && cameras.length > 0) {
          ...
          await newScanner.start(
            ...
            (errorMessage) => {
              // Ignore standard scan failures (it scans continuously)
            }
          );
        } else {
          setHasCameras(false);
        ...
        setHasCameras(false);
        setError(t.cameraPermissionDenied || 'Camera permission denied or unavailable');
    ...
    <div className={`fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex flex-col items-center justify-center p-4 ${language === 'ar' ? 'font-arabic' : ''}`}>
      <div className="bg-white dark:bg-gray-900 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden relative">
        ...
        <div className="p-6 text-center border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
          ...
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">{t.scanBarcode || 'Scan Barcode / QR'}</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t.scanInstruction || 'Point your camera at a product label'}</p>
        </div>
